...
if os.path.exists('dist') and os.path.isdir('dist'):
    if os.path.exists('dist/剑星MOD管理器'):
        dest_dir = os.path.join('1.55', 'dist', '剑星MOD管理器')
        os.makedirs(os.path.dirname(dest_dir), exist_ok=True)
        if os.path.exists(dest_dir):
            shutil.rmtree(dest_dir)
        shutil.copytree('dist/剑星MOD管理器', dest_dir)
        print("复制已打包的MOD管理器到1.55/dist目录成功")
    else:
        print("dist/剑星MOD管理器目录不存在")
else:
    print("dist目录不存在")
