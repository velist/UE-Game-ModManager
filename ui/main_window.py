...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.edit_mode_cb.setVisible(False)  # 隐藏编辑模式复选框
        ...
        if event.mimeData().hasFormat('application/x-qabstractitemmodeldatalist'):
            ...
            for item in selected_items:
                ...
                if mod_info:
                    ...
                    self.config.update_mod(mod_id, mod_info)
                    processed_count += 1
                    print(f"[调试] on_tree_drop_event: 更新MOD {mod_id} 的分类: {old_category} -> {target_category}")
                else:
                    print(f"[警告] on_tree_drop_event: 找不到MOD {mod_id} 的信息")
