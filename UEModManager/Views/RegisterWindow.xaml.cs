...
namespace UEModManager.Views
{
    public partial class RegisterWindow : Window
    {
        ...
        private async System.Threading.Tasks.Task PerformRegistrationAsync(string email, string password, string? username)
        {
            try
            {
                ShowLoading(true, "正在注册...");

                var result = await _unifiedAuthService.RegisterAsync(email, password, username);
                ...
                {
                    ShowMessage("注册失败: " + result.Message, "注册错误");

                    if (result.Message.Contains("邮箱"))
                    {
                        EmailTextBox.Focus();
                    }
            ...
            {
                _logger?.LogError(ex, $"注册异常: {email}");
        ...
        {
            CyberMessageBox.Show(this, message, title, MessageBoxButton.OK, MessageBoxImage.Information);
        }
