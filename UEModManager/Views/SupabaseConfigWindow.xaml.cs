    ...
    public partial class SupabaseConfigWindow : Window
    {
        ...
        private void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                CyberMessageBox.Show(this, UseDemoMode ?
                    "已保存为演示模式，将使用本地存储。" :
                    "账号服务设置已保存！",
                    "保存成功",
                    MessageBoxButton.OK,
            ...
            {
                CyberMessageBox.Show(this, $"保存配置失败：{ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void ApplyLocalization()
        {
            ...
            {
                {"账号服务设置","Account Service Settings"},
                {"配置云端认证服务","Configure Cloud Authentication"},
                {"使用离线账号模式（不连接云端）","Use offline account mode (no cloud connection)"},
                {"离线账号模式会把用户信息保存在本机，不需要网络连接。","Offline account mode stores users locally. No network required."},
                {"云端账号服务设置","Cloud Account Service Settings"},
                {"项目URL：","Project URL:"},
                {"访问密钥：","Access Key:"},
            ...
            this.Title = toEnglish ? "Account Service Settings" : "账号服务设置";
        }
